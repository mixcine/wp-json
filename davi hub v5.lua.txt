...
local Window = redzlib:MakeWindow({
    Title = "Davi Hub | Brookhaven RP 🏡 BETA",
    SubTitle = "por davi.scripts ",
    SaveFolder = "testando Davi Hub"
  })

  Window:AddMinimizeButton({
    Button = { Image = "rbxassetid://81944994548447", BackgroundTransparency = 0 },
    Corner = { CornerRadius = UDim.new(35, 1) },
})



local Tab1 = Window:MakeTab({"Créditos", "info"})
local Tab2= Window:MakeTab({"Jogador", "fun"})
...
local Tab7 = Window:MakeTab({"Tocar Musica Para Todos", "radio"})    
local Tab8 = Window:MakeTab({"Musica", "music"}) 
local Tab9 = Window:MakeTab({"Trollar", "skull"}) 
local Tab10 = Window:MakeTab({"Scripts Externos", "script"})




--------------------------------------------------------------------------------------------------------------------------------
                                                   -- === Tab1:AddButton( 1: credits === --
---------------------------------------------------------------------------------------------------------------------------------
Tab1:AddSection({"Créditos do Hub"})

Tab1:AddDiscordInvite({
    Name = "Davi Hub",
    Description = "Entre No Servidor Discord Do Melhor Hub Feito Por Brasileiros Para Brookhaven RP.",
    Logo = "rbxassetid://81944994548447",
    Invite = "https://discord.gg/CeTUaMCWXP",
})
...
local Paragraph = Tab1:AddParagraph({"Criador", "davi.scripts "})

local Paragraph = Tab1:AddParagraph({"Créditos", "sho.orn_ , davi.scripts"})


  
  Tab1:AddButton({
    Name = " - Copiar Nome De Usuario Roblox Do Desenvolvedor",
    Callback = function()
      setclipboard("proplayer800C") -- Copia o @
    end
  })
 Tab1:AddButton({
    Name = " - Copiar Nome De Usuario Discord Do Desenvolvedor",
    Callback = function()
      setclipboard("Davi.scripts") -- Copia o @
    end
  })
  
  Tab1:AddButton({
    Name = " - Copiar Link Do Canal Do Youtube Do Desenvolvedor",
    Callback = function()
      setclipboard("davi scripts official") -- Copia o link do canal
    end
  })
  
  Tab1:AddButton({
    Name = " - Copiar Link Do Servidor Discord Do Desenvolvedor",
    Callback = function()
      setclipboard("https://discord.gg/zdsrxB7qhM") -- Copia o link do servidor
    end
  })
...
Tab2:AddTextBox({
    ...
    PlaceholderText = "ex: lo → davi.scripts",
...
Tab9:AddButton({"ativar o touch interests (pode ser bugado)", function()
    --[[
	WARNING: Heads up! This script has not been verified by ScriptBlox. Use at your own risk!
]]
local cloneref = cloneref or function(v) return v end
local Players = cloneref(game:FindService("Players") or game:GetService("Players"))
local Plr = cloneref(Players.LocalPlayer)
local Char = cloneref(Plr.Character) or Plr.CharacterAdded:Wait()
local Hum = cloneref(Char:FindFirstChild("Humanoid")) or Char:WaitForChild("Humanoid")
local Hrp = cloneref(Char:FindFirstChild("HumanoidRootPart")) or Char:WaitForChild("HumanoidRootPart")
local workspace = cloneref(game:FindService("Workspace") or game:GetService("Workspace"))
...
local StarterGui = game:GetService("StarterGui")
local LocalPlayer = Players.LocalPlayer
local Event = ReplicatedStorage:WaitForChild("RE"):WaitForChild("1Clea1rTool1s")
local testing = false
local testConn = nil
local fireInterval = 0 -- Fire every 0.5 seconds to avoid spamming
local fireCount = 0
local maxFires = 0 -- Limit to 50 fires for safety

-- Function to show notification (since original showNotification is undefined)
...
-- Start test button
Tab10:AddButton({"Iniciar Teste", function()
    if testing then
        showNotification("Teste Já Ativo", "O teste já está em execução.", 3)
        return
    end
    testing = true
    fireCount = 0
    showNotification("Teste Iniciado", "Iniciando teste controlado do FireServer.", 3)
    
    testConn = RunService.Heartbeat:Connect(function()
        if not testing or fireCount >= maxFires then
            if testConn then
                testConn:Disconnect()
                testConn = nil
                testing = false
                showNotification("Teste Finalizado", "Teste concluído ou interrompido.", 5)
            end
            return
        end
        
        -- Rate-limited FireServer call
        if fireCount % math.floor(fireInterval / RunService.Heartbeat:Wait()) == 0 then
            pcall(function()
                Event:FireServer("TurnOnGreenCheckMark", "Basketball")
                fireCount = fireCount + 1
                print("FireServer chamado: " .. fireCount .. " vezes")
            end)
        end
    end)
end})
...
-- Stop test button
Tab10:AddButton({"Parar Teste", function()
    if not testing then
        showNotification("Nenhum Teste Ativo", "Nenhum teste está em execução.", 3)
        return
    end
    testing = false
    if testConn then
    ...
    showNotification("Teste Parado", "O teste foi desativado. Total de chamadas: " .. fireCount, 5)
end})
